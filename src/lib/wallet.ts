 ...
 * Wallet layer.
 *
 * v0: hot wallet fallback (loads private key from .env, signs Jupiter
 *     orders directly). Fast to ship for the MVP demo.
...
function hotKeypair(): Keypair {
  ...
  if (!sk) throw new Error("SOLANA_WALLET_PRIVATE_KEY missing in .env");
